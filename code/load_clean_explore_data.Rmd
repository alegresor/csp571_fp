...
## Detecting outliers
```{r}
mod <- lm(salary ~ .-salary, data=df_final)
cooksd <- cooks.distance(mod)

plot(cooksd, pch="*", cex=2, main="Outliers")  # plot cook's distance
abline(h = 4*mean(cooksd, na.rm=T), col="red")  # add cutoff line. Those observations that have a cook’s distance greater than 4 times the mean may be classified as influential. 
...
df_final[influential, ]

# Treating the outliers by capping them.
# x <- df_final$salary
...
p_formula
p_selection_model <- ols(p_formula, data = p_train_df)
p_selection_model
...
p_subset_formula
p_subset_lm <- lm(p_subset_formula , data=p_train_df)
summary(p_subset_lm)
...
```{r message=F}
library(rms)
c_x_vars <- names(c_train_df)[!(names(c_train_df)%in%c('salary','name','2P','2PA','PTS','TRB'))]
# 2P, 2PA, PTS, and TRB were causing singularity in predictor matrix, so they were dropped
c_formula <- get_salary_formula(c_x_vars)
c_formula
c_selection_model <- ols(c_formula, data = c_train_df)
c_selection_model
...
c_subset_formula
c_subset_lm <- lm(c_subset_formula , data=c_train_df)
summary(c_subset_lm)
