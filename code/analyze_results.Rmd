...
## Load Results from SLR Model
```{r}
df <- read.csv('../data/predictions/slr.csv')
head(df)
```
...
ggplot(df,aes(x=salary,y=salary_hat)) + 
  geom_point() +
  geom_smooth()
ggsave("../figures/slr_salary_vs_prediction_scatter.png", width=10, height=7)
```
...
ggplot(df, aes(x=salary,y=residual)) + 
  geom_point() + 
  geom_smooth()
ggsave("../figures/slr_salary_vs_residual_scatter.png", width=10, height=7)
```
