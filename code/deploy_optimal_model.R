...
library(glmnet)
library(methods)
model <- readRDS(file='../data/optimal_model/elasticnet/model.rds')
...
#' @param PTS points
#' @param out NBA 2K outside rating
#' @param ovr NBA 2K overall rating
#' @param ins NBA 2K inside rating
#' @param pla NBA 2K playmaking rating
#' @param ath NBA 2K athleticism rating
#' @param def NBA 2K defensive rating
#' @param reb NBA 2K rebouding rating
#' @post /predict_salary
function(Age=0, G=0, GS=0, MP=0, PER=0, TS.=0, X3PAr=0, FTr=0, ORB.=0, DRB.=0, TRB.=0,
         ...
         BLK=0, TOV=0, PF=0, PTS=0, out=0, ovr=0, ins=0, pla=0, ath=0, def=0, reb=0){
  vec <- c(Age,G,GS,MP,PER,TS.,X3PAr,FTr,ORB.,DRB.,TRB.,AST.,STL.,BLK.,TOV.,USG.,
             ...
             X2P.,eFG.,FT,FTA,FT.,ORB,DRB,TRB,AST,STL,BLK,TOV,PF,PTS,out,ovr,ins,pla,ath,def,reb)
  mat <- matrix(as.double(vec),1,length(vec))
  mat <- data.matrix(mat)
  val <- predict(model,s=model$lambda.min,newx=mat)
  return(val)}